...
// Last update Sun Apr  9 11:02:15 2017 Sylvain Chaugny
//
...
int		arcade::snake::GameSnake::play(arcade::api::IEvents const &event)
{
  ...
  if (_first_play)
    ...
    {
      if (_menu.isSetupMenuActive())
	{
	  if (_menu.displaySetup(event, _boardDimensions))
	    restart();
	}
      else
	{
...
{
  _food_cell.setRandomCoordinates(_boardDimensions);
  _random_cell.setRandomCoordinates(_boardDimensions);
  _random_cell.setRandomEffect();
  _game_over = false;
  _player.clear();
  _turn_points.clear();
  _score = 0;
...
void		arcade::snake::GameSnake::addSnakeNode(int multiple_add)
{
  SnakeCell	last;
  SnakeCell	cell;
  arcade::utl::Position pos;
  ...
  if (multiple_add)
    {
      if (last.getDirection() == arcade::api::IEvents::Key::LEFT)
	pos.x++;
      else if (last.getDirection() == arcade::api::IEvents::Key::RIGHT)
	pos.x--;

      if (last.getDirection() == arcade::api::IEvents::Key::UP)
	pos.y++;
      else if (last.getDirection() == arcade::api::IEvents::Key::DOWN)
	pos.y--;
    }
  ...
  if (cell.getSpriteID() == -1)
    throw arcade::snake::SnakeException("Unable to load texture");
...
void		arcade::snake::GameSnake::displayPlayer()
{
  ...
  if (_random_cell.isDisplayable())
    graphic->drawSprite(_random_cell.getPos(), powerup_id, arcade::utl::Color(0, 255, 255));
}
...
void		arcade::snake::GameSnake::updatePlayer(arcade::api::IEvents const &event)
{
  ...
  if (event.keyPressed(arcade::api::IEvents::Key::UP) && backup != arcade::api::IEvents::Key::UP)
    _player.back().setDirection(arcade::api::IEvents::Key::UP);
  if (event.keyPressed(arcade::api::IEvents::Key::DOWN) && backup != arcade::api::IEvents::Key::DOWN)
    _player.back().setDirection(arcade::api::IEvents::Key::DOWN);
  if (event.keyPressed(arcade::api::IEvents::Key::LEFT) && backup != arcade::api::IEvents::Key::LEFT)
    _player.back().setDirection(arcade::api::IEvents::Key::LEFT);
  if (event.keyPressed(arcade::api::IEvents::Key::RIGHT) && backup != arcade::api::IEvents::Key::RIGHT)
...
void		arcade::snake::GameSnake::checkFoodEating()
{
  ...
  if (_random_cell.isDisplayable() && _player.back().getPosX() == _random_cell.getPosX() &&
      _player.back().getPosY() == _random_cell.getPosY())
    {
      effect = _random_cell.getEffect();
      count = 0;
      while (count < ((effect < 0) ? -1 * effect : effect))
	{
	  if (effect < 0 && _player.size() > 4)
	    _player.erase(_player.begin());
	  else
	    addSnakeNode((effect > 1 && count) ? 1 : 0);
	  count++;
	}
      _score += effect * 10;
      _random_cell.setRandomCoordinates(_boardDimensions);
